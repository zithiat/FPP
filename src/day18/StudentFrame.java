...
public class StudentFrame extends javax.swing.JFrame {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 1736726717905171609L;
//	private JScrollPane JScrollPane1;
	...
	void Bshowdata_actionPerformed(java.awt.event.ActionEvent event) {
		...
		try {
			while (rs.next()) {
				...
				gpa = rs.getInt("GPA");
				gpaList.add(gpa);
				System.out.println("Student ID is " + id + "   Student Name is " + name + "   GPA is " + gpa);
		...
		System.out.println("Average GPA of all students:" + (double) total / gpaList.size());
	...
	void DoUpdate(dbconnection db) {
		System.out.println("Insert new record");
		String insertQuery = "Insert into StudentInfo values (\"999-990\", \"Newton\", 23, 4)";
		db.DoUpdate(insertQuery);
		
		ResultSet rs;
		rs = db.DoQuery("Select * from StudentInfo");
		try {
			while (rs.next()) {
				System.out.println(rs.getString("StudentID") + ", " + rs.getString("StudentName") + ", " + rs.getString("Age") + rs.getString("GPA"));			}
		} catch (SQLException ex) {
			System.err.println("error in database connection");
		}
		
		System.out.println();
		System.out.println("Delete new record");
		
		String deleteQuery = "Delete from StudentInfo where StudentID = \"999-990\"";
		db.DoUpdate(deleteQuery);
		rs = db.DoQuery("Select * from StudentInfo");
		try {
			while (rs.next()) {
				System.out.println(rs.getString("StudentID") + ", " + rs.getString("StudentName") + ", " + rs.getString("Age") + rs.getString("GPA"));			}
